...
export function TemplateManager({ initialValues, allowedTemplates, subscriptionPlan, subscriptionStatus }: { initialValues: PlatformValues; allowedTemplates: BusinessTemplateId[]; subscriptionPlan: string; subscriptionStatus: "ACTIVE" | "PAST_DUE" | "CANCELLED" }) {
  ...
  return <div className="mt-8 space-y-5">
    <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--brand)]">Business platform</p><h2 className="mt-1 text-xl font-bold text-slate-950">Template, modules, and language</h2><p className="mt-1 text-sm text-slate-500">Configure the whole workspace from one shared platform. Applying a template never removes products, customers, transactions, or existing categories.</p></div>
...
function TemplateCard({ template, active, locked, busy, onApply }: { template: BusinessTemplate; active: boolean; locked: boolean; busy: string | null; onApply: (id: BusinessTemplateId) => Promise<void> }) { return <article className={`flex flex-col rounded-xl border p-4 ${active ? "border-[var(--brand)] bg-[var(--brand-soft)]" : "border-[var(--border)]"} ${locked ? "bg-slate-50/70 opacity-70" : ""}`}><div className="flex items-start justify-between gap-3"><div><h4 className="text-sm font-bold text-slate-900">{template.name}</h4><p className="mt-2 text-[11px] leading-5 text-slate-500">{template.description}</p></div>{active ? <span className="flex shrink-0 items-center gap-1 rounded-full bg-white px-2 py-1 text-[10px] font-bold text-[var(--brand)]"><Check size={12} /> Active</span> : locked ? <span className="flex shrink-0 items-center gap-1 rounded-full bg-slate-200 px-2 py-1 text-[10px] font-bold text-slate-600"><LockKeyhole size={11} /> Locked</span> : null}</div><div className="mt-3 rounded-lg bg-slate-50 px-3 py-2 text-[10px] leading-5 text-slate-500"><span className="font-bold text-slate-700">Example:</span> {template.examples.catalogItem}<br /><span className="font-mono">{template.examples.sku}</span> · {template.examples.category}</div><p className="mt-3 text-[10px] font-semibold text-slate-500">{template.sampleCatalog.length ? `${template.sampleCatalog.length} template items · ` : ""}{template.suggestedCategories.length ? `Adds missing categories: ${template.suggestedCategories.join(", ")}` : "No suggested categories are added."}</p><Button type="button" size="sm" variant={active ? "secondary" : "primary"} className="mt-4" disabled={busy !== null || locked} onClick={() => onApply(template.id)}>{locked ? "Not in subscription" : busy === template.id ? "Applying…" : active ? template.sampleCatalog.length ? "Refresh template menu" : "Reapply template" : "Apply template"}</Button></article>; }
function Toggle({ label, description, checked, disabled, onChange }: { label: string; description?: string; checked: boolean; disabled?: boolean; onChange: (checked: boolean) => void }) { return <label className={`flex items-center justify-between gap-4 rounded-xl border border-[var(--border)] p-4 ${disabled ? "cursor-not-allowed bg-slate-50 opacity-65" : "cursor-pointer"}`}><span><span className="block text-xs font-bold text-slate-800">{label}</span>{description ? <span className="mt-1 block text-[11px] leading-4 text-slate-500">{description}</span> : null}</span><input type="checkbox" checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} className="size-4 shrink-0 accent-[var(--brand)]" /></label>; }
