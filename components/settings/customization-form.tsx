...
export function CustomizationForm({ initialValues }: { initialValues: CustomizationValues }) {
  ...
  const [surfaceColor, setSurfaceColor] = useState(initialValues.surfaceColor);
  ...
  async function save() {
    ...
    try {
      const result = await updateCustomizationSettings({
        workspaceName, workspaceTagline, brandColor, brandDarkColor, brandSoftColor, sidebarColor, backgroundColor, surfaceColor, receiptLayout, dashboardWidgets: widgets, customerCustomFields: customerFields, productCustomFields: productFields,
      });
...
function Field({ label, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) { return <label><span className="mb-2 block text-xs font-bold text-[var(--ink-soft)]">{label}</span><input {...props} className="h-11 w-full rounded-lg border border-[var(--border)] px-3 text-sm outline-none focus:border-[var(--brand)] focus:ring-2 focus:ring-[var(--brand-soft)]" /></label>; }
function ColorField({ label, name, value, onChange }: { label: string; name: string; value: string; onChange: (value: string) => void }) { const validValue = /^#[0-9a-f]{6}$/i.test(value) ? value : "#000000"; return <label><span className="mb-2 block text-xs font-bold text-[var(--ink-soft)]">{label}</span><span className="flex h-11 items-center gap-3 rounded-lg border border-[var(--border)] px-3 focus-within:border-[var(--brand)]"><input type="color" aria-label={`${label} color picker`} value={validValue} onChange={(event) => onChange(event.target.value)} className="size-7 cursor-pointer rounded border-0 bg-transparent p-0" /><input name={name} aria-label={`${label} hex value`} value={value} onChange={(event) => onChange(event.target.value)} pattern="#[0-9a-fA-F]{6}" maxLength={7} required className="min-w-0 flex-1 bg-transparent font-mono text-xs text-[var(--ink-soft)] outline-none" /></span></label>; }
function Toggle({ label, description, checked, onChange }: { label: string; description?: string; checked: boolean; onChange: (checked: boolean) => void }) { return <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-[var(--border)] p-4"><span><span className="block text-xs font-bold text-[var(--foreground)]">{label}</span>{description ? <span className="mt-1 block text-[11px] leading-4 text-[var(--muted)]">{description}</span> : null}</span><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="size-4 shrink-0 accent-[var(--brand)]" /></label>; }
...
function CustomFieldEditor({ title, fields, onChange }: { title: string; fields: CustomFieldDefinition[]; onChange: (fields: CustomFieldDefinition[]) => void }) {
  ...
  return <div><div className="flex items-center justify-between"><h4 className="text-xs font-bold text-[var(--foreground)]">{title}</h4><Button type="button" size="sm" variant="secondary" onClick={addField} disabled={fields.length >= 12}><Plus size={14} /> Add field</Button></div><div className="mt-3 space-y-3">{fields.map((field) => <div key={field.id} className="rounded-xl border border-[var(--border)] bg-[var(--surface-subtle)] p-3"><div className="grid gap-2 sm:grid-cols-[1fr_120px_32px]"><input value={field.label} onChange={(event) => onChange(fields.map((item) => item.id === field.id ? { ...item, label: event.target.value } : item))} placeholder="Field label" maxLength={60} className="h-9 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 text-xs outline-none" /><select value={field.type} onChange={(event) => onChange(fields.map((item) => item.id === field.id ? { ...item, type: event.target.value as CustomFieldDefinition["type"] } : item))} className="h-9 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-2 text-xs"><option value="text">Text</option><option value="number">Number</option><option value="date">Date</option><option value="select">Dropdown</option></select><button type="button" onClick={() => onChange(fields.filter((item) => item.id !== field.id))} aria-label={`Remove ${field.label || "field"}`} className="grid size-8 place-items-center text-[var(--muted)] hover:text-red-600"><Trash2 size={15} /></button></div>{field.type === "select" ? <input value={field.options.join(", ")} onChange={(event) => onChange(fields.map((item) => item.id === field.id ? { ...item, options: event.target.value.split(",").map((option) => option.trim()).filter(Boolean) } : item))} placeholder="Options separated by commas" className="mt-2 h-9 w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 text-xs outline-none" /> : null}<label className="mt-2 flex items-center gap-2 text-[11px] text-[var(--ink-soft)]"><input type="checkbox" checked={field.required} onChange={(event) => onChange(fields.map((item) => item.id === field.id ? { ...item, required: event.target.checked } : item))} className="accent-[var(--brand)]" /> Required</label></div>)}{!fields.length ? <p className="rounded-xl border border-dashed border-[var(--border)] px-4 py-8 text-center text-xs text-[var(--muted)]">No custom fields yet.</p> : null}</div></div>;
}
