...
export function SettingsForm({ initialValues }: { initialValues: SettingsValues }) {
  ...
  return <form action={save} className="grid gap-5 xl:grid-cols-[220px_1fr]">
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-end gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-5 py-4">{saved ? <span className="flex items-center gap-1.5 text-xs font-bold text-emerald-700"><Check size={15} /> Settings saved</span> : <span className="text-xs text-[var(--muted)]">Changes apply across the workspace.</span>}<Button type="submit" disabled={saving}>{saving ? "Saving…" : "Save changes"}</Button></div>
    </div>
  </form>;
}
