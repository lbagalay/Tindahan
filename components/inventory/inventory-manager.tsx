...
import { deleteCatalogItem } from "@/app/actions/management";
...
export function InventoryManager({ initialProducts, initialMovements = inventoryMovements, initialMovementsToday = 0, currency = "PHP" }: { initialProducts?: DemoProduct[]; initialMovements?: DemoMovement[]; initialMovementsToday?: number; currency?: string }) {
  ...
  async function removeProduct(product: DemoProduct) {
    if (!window.confirm(`Remove “${product.name}” from inventory? This cannot be undone.`)) return;
    setRemovingId(product.id);
    try {
      const result = await deleteCatalogItem({ id: product.id });
      if (!result.ok) { window.alert(result.error); return; }
      setProducts((current) => current.filter((item) => item.id !== product.id));
      if (result.archived) window.alert("This item has past sales, so it was archived (hidden from the menu and counter) instead of deleted, to keep your reports accurate.");
    } catch {
      window.alert("Connection lost while removing. Check your internet and try again.");
    } finally {
      setRemovingId(null);
    }
  }
