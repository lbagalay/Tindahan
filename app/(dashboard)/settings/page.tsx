...
export default async function SettingsPage() {
  ...
  const customization: CustomizationValues = { workspaceName: business.settings?.workspaceName ?? "Tindahan", workspaceTagline: business.settings?.workspaceTagline ?? "Business OS", brandColor: business.settings?.brandColor ?? "#176b4d", brandDarkColor: business.settings?.brandDarkColor ?? "#0e5038", brandSoftColor: business.settings?.brandSoftColor ?? "#e4f2eb", sidebarColor: business.settings?.sidebarColor ?? "#123d2e", backgroundColor: business.settings?.backgroundColor ?? "#f6efe0", surfaceColor: business.settings?.surfaceColor ?? "#fffbf2", receiptLayout: business.settings?.receiptLayout === "COMPACT" ? "COMPACT" : "DETAILED", dashboardWidgets: readDashboardWidgets(business.settings?.dashboardWidgets), customerCustomFields: readCustomFields(business.settings?.customerCustomFields), productCustomFields: readCustomFields(business.settings?.productCustomFields) };
