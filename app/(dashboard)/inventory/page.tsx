...
export default async function InventoryPage() {
  ...
  // Recipe-linked items no longer carry their own stock — they live on the Ingredients page instead.
  const [records, movementRecords, movementsToday, settings] = await Promise.all([prisma.product.findMany({ where: { businessId: session.user.businessId, type: "PRODUCT", recipeItems: { none: {} }, ...catalogScope }, include: { category: true }, orderBy: { name: "asc" } }), prisma.inventoryMovement.findMany({ where: { businessId: session.user.businessId, product: catalogScope }, include: { product: true, createdBy: true }, orderBy: { createdAt: "desc" }, take: 50 }), prisma.inventoryMovement.count({ where: { businessId: session.user.businessId, product: catalogScope, createdAt: { gte: today } } }), prisma.businessSettings.findUnique({ where: { businessId: session.user.businessId }, select: { currency: true } })]);
